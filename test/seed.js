        ...
        context('# buy', () => {
            context('» generics', () => {
                ...
                it('it cannot buy 0 seeds', async () => {
                    await expectRevert(setup.seed.buy(zero.toString(), {from:buyer1}), 'Seed: amountVestedPerSecond > 0');
                });
                ...
                it('updates the remaining seeds to distribution', async () => {
                    expect((await setup.seed.seedRemainder()).toString()).to
                        .equal(seedForDistribution.sub(new BN(buySeedAmount)).toString());
                });
                it('updates the remaining seeds for fee', async () => {
                    expect((await setup.seed.feeSeedRemainder()).toString()).to
                        .equal(seedForFee.sub(new BN(buySeedFee)).toString());
                });
                ...
                it('updates the remaining seeds for fee after another buy', async () => {
                    expect((await setup.seed.feeSeedRemainder()).toString()).to
                        .equal(seedForFee.sub(new BN(buySeedFee).mul(twoBN)).toString());
                });
                it('return totalClaimed == 0', async () => {
                    expect((await setup.seed.getTotalClaimed(buyer1)).toString()).to.equal(zero.toString());
                });
            });
        });
        context('# claim', () => {
            context('» generics', () => {
                ...
                it('calculates correct claim', async () => {
                    ...
                    const expectedClaim = ((await time.latest()).sub(startTime)).mul((new BN(buySeedAmount).mul(twoBN)).div(new BN(vestingDuration)));
                ...
                it('it cannot claim if not vested', async () => {
                    await expectRevert(setup.seed.claim(buyer2, new BN(buySeedAmount).mul(twoBN).add(new BN(one)), {from:buyer1}),
                        "Seed: amount claimable is 0"
                    );
                });
                it('it cannot claim more than claimable amount', async () => {
                    await expectRevert(setup.seed.claim( buyer1, new BN(buySeedAmount).mul(twoBN).add(new BN(one)), {from:buyer1}),
                ...
                it('updates claim', async () => {
                    expect((await setup.seed.getTotalClaimed(buyer1)).toString()).to.equal(claimAmount.toString());
                });
                it('updates fee claimed', async () => {
                    expect((await setup.seed.getFeeClaimed(buyer1)).toString()).to.equal(feeAmount.toString());
                });
                it('funds dao with fee', async () => {
                    expect((await seedToken.balanceOf(setup.organization.avatar.address)).toString()).to
                        .equal(feeAmount.toString());
                });
            ...
            context('» claim after vesting duration', async () => {
                ...
                it("it claims all the fee for a buyer's claim", async () => {
                    const fee = await setup.data.seed.getFee(buyer2);
                    const feeClaimed = await setup.data.seed.getFeeClaimed(buyer2);
                ...
                it('funds DAO with all the fee', async () => {
                    const fee = await setup.data.seed.getFee(buyer2);
                    expect((await seedToken.balanceOf(setup.organization.avatar.address)).toString()).to
                        .equal(fee.add(await setup.seed.getFeeClaimed(buyer1)).toString());
                });
            });
        });
        context('# retrieveFundingTokens', () => {
            context('» generics', () => {
                ...
                it('it cannot return funding tokens if not bought', async () => {
                    await expectRevert(setup.data.seed.retrieveFundingTokens({from:buyer1}),
                ...
                it('clears `fee` mapping', async () => {
                    expect((await setup.data.seed.getFee(buyer2)).toString()).to.equal(zero.toString());
                });
                it('clears `tokenLock.amount`', async () => {
                    expect((await setup.data.seed.getSeedAmount(buyer2)).toString()).to.equal(zero.toString());
                });
                it('updates `feeSeedRemainder` ', async () => {
                    expect((await setup.data.seed.feeSeedRemainder()).toString()).to
                        .equal(seedForFee.toString());
                });
                it('updates remaining seeds', async () => {
                    expect((await setup.data.seed.seedRemainder()).toString()).to
                        .equal(seedForDistribution.toString());
                });
            ...
            context('» getAmount', () => {
                it('returns correct amount', async () => {
                    expect((await setup.seed.getSeedAmount(buyer1)).toString()).to
                        .equal(((new BN(buySeedAmount)).mul(twoBN)).toString());
                });
            });
        ...
        context('# admin functions', () => {
            ...
            context('» withdraw', () => {
                ...
                it('can withdraw after minimum funding amount is met', async () => {
                    ...
                    expect((await fundingToken.balanceOf(setup.data.seed.address)).toString()).to.equal(zero.toString());
